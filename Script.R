####Detection of novel and resident marine species using environmental DNA metabarcoding of sediment and water####
####Luke E. Holman####
...
#Set some variables, we want to set 3 reads mapped to an OTU as a minimum  
minreads <- 3
items <- NULL
...
#Here is a little function we use later on
counter <- function(x){
  temp <- x
  temp[temp>0] <- 1
  return(sum(temp))
...
#Load up sediment metadata 
sedimentID <- read.csv(file="sedimentdata.csv")

#### Data Cleaning ####

#Lets clean all the different datasets we have generated
files <- list.files("raw.data/")

#Loop over the files
for (file in  files){
  
  rawdat <-read.csv(paste("raw.data/",file,sep=""))
...
#####Question 1 - sample storage - Longmire's or Freezing ####

#First we need to rarefy our data to make sensible comparisons between sites/conditions
#Read in data
  Preserv18S <- read.table(file="cleaned/Cleaned.Zhanlulu.unoise3.csv",row.names = 1,sep=",",header=TRUE)
...
PreservCOI <- read.table(file="cleaned/Cleaned.Leraylulu.unoise3.csv",row.names = 1,sep=",",header=TRUE)
...
####Question 2 - comparing water and sediment ####

##Now we have moved on from comparing longmires and frozen lets re-rarefy the COI data without the longmires sampes 
##The limiting sample in the 18S dataset is a frozen sample so no need to rarefy there! 
...
#Statistics 2-way anova testing for effect of site, eDNA sample type adn an interaction on OTU richness 
#18S
...
rSedWater18SOTU_r$OTUnum[rSedWater18SOTU_r$cond=="N"]
rSedWater18SOTU_r$OTUnum[rSedWater18SOTU_r$cond=="S"]
...
#lets pull in the taxonomy (BLAST best hit adjusted!) genrated using the below calls
...
colnames(SedWatertaxa)[12] <- "Other"
...
barplot(SedWatertaxaP[3:1,],col=c("#8c510a","#f6e8c3","#01665e"),las=2)
...
##We are using only the COI data as 18s does not have enough resolution.

##First lets group the replicates per Site/treatment 
...
##Excellent - now we have really lovely charts of summed reads per species/site

tempdatCOI <- invadersCOI[-9]
tempdatCOI$species<- paste(invadersCOI$species,invadersCOI$status,sep=",")

invadersOUTPUT <- aggregate(.~species,data=tempdatCOI,FUN=sum)
invadersOUTPUT.preconcat <- invadersOUTPUT
invadersOUTPUT.preconcat <- separate(invadersOUTPUT.preconcat, species, c("ID","Status"),sep=",")
invadersOUTPUT.preconcat$Status[invadersOUTPUT.preconcat$Status=="NA"] <- "IDonly"
invadersOUTPUT$PQ <- rowSums(cbind(invadersOUTPUT$PQ.W,invadersOUTPUT$PQ.S))
invadersOUTPUT$TQ <- rowSums(cbind(invadersOUTPUT$TQ.W,invadersOUTPUT$TQ.S))
invadersOUTPUT$TB <- rowSums(cbind(invadersOUTPUT$TB.W,invadersOUTPUT$TB.S))
invadersOUTPUT$HH <- invadersOUTPUT$HH.W
invadersOUTPUT <- cbind(invadersOUTPUT[1],invadersOUTPUT[9:12])

invadersOUTPUT <- separate(invadersOUTPUT, species, c("ID","Status"),sep=",")
invadersOUTPUT$Status[invadersOUTPUT$Status=="NA"] <- "IDonly"

invadersOUTPUT[3:6][invadersOUTPUT[3:6] > 0] <- 1

invadersOUTPUT <- invadersOUTPUT[rowSums(invadersOUTPUT[3:6])>0,]
invadersOUTPUT$Status[invadersOUTPUT$Status=="ID"] <- "IDonly"

noInvaders<- aggregate(.~Status, data=invadersOUTPUT[2:6], FUN=sum) 
...
mean(colSums(invadersOUTPUT[invadersOUTPUT$Status=="UK",3:6]))
sd(colSums(invadersOUTPUT[invadersOUTPUT$Status=="UK",3:6]))
#Mean number of global invaders per site
mean(colSums(invadersOUTPUT[invadersOUTPUT$Status=="Global",3:6]))
sd(colSums(invadersOUTPUT[invadersOUTPUT$Status=="Global",3:6]))

#Total number of invasev species
length(invadersOUTPUT$ID[invadersOUTPUT$Status=="UK"])
length(invadersOUTPUT$ID[invadersOUTPUT$Status=="Global"])
length(invadersOUTPUT$ID[invadersOUTPUT$Status=="IDonly"])

#Now lets add material data to this chart
invadersOUTPUT$medium <- rep(NA,length(invadersOUTPUT$ID))

for (invasivespecies in 1:length(invadersOUTPUT$ID)){
  
  #This 'if' ladder annotates the medium
  
  runningW <- sum(invadersOUTPUT.preconcat$TQ.W[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]],invadersOUTPUT.preconcat$PQ.W[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]],invadersOUTPUT.preconcat$TB.W[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]],invadersOUTPUT.preconcat$HH.W[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]])
  runningS <- sum(invadersOUTPUT.preconcat$TQ.S[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]],invadersOUTPUT.preconcat$PQ.S[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]],invadersOUTPUT.preconcat$TB.S[invadersOUTPUT.preconcat$ID==invadersOUTPUT$ID[invasivespecies]])
  if(runningW>0 & runningS>0){
    invadersOUTPUT$medium[invasivespecies] <- "Both"
  } else if (runningW>0 & runningS==0){
    invadersOUTPUT$medium[invasivespecies] <- "Water"
  } else if (runningW==0 & runningS>0){
    invadersOUTPUT$medium[invasivespecies] <- "Sediment"
  }

}


for (species in invadersBOTH$ID){
  rownames(annotatedCOI$assignment[annotatedCOI$assignment==species])
}

subset.invadersBOTH <- invadersBOTH[invadersBOTH$Status!="IDonly",]


test <- rownames(annotatedCOI)

subset <- annotatedCOI[annotatedCOI$assignment %in% subset.invadersBOTH$ID,]
subset$seq <- rep(NA,65)
counter <- 1
for (OTU in rownames(subset)){
  subset$seq[counter] <- as.character(COIseqs[OTU])
  counter <- counter+1
}

write.csv(data.frame("OTUs"=rownames(subset),"Seq"=subset$seq,"Spp"=subset$assignment),file="test.csv")


COIseqs <- seqinr::read.fasta("taxonomy/Leray.unoise3.OTUs.fa",as.string = TRUE)

write.csv(invadersOUTPUT[invadersOUTPUT$Status!="IDonly",],file="taxonomy/finalINVADERSforcheck.csv")



##Additional unused figures

Fig1.4nonnaitvesCOI <-table(invadersOUTPUT$Status[invadersOUTPUT$marker=="COI" | invadersOUTPUT$marker=="Both"],invadersOUTPUT$medium[invadersOUTPUT$marker=="COI" | invadersOUTPUT$marker=="Both"])
...
Fig1.5nonnaitves <-table(invadersOUTPUT$Status,invadersOUTPUT$medium)
